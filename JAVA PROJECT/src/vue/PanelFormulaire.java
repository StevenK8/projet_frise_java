	...
	public void actionPerformed(ActionEvent parEvt) {
		if(parEvt.getSource()==ajout){
			if(saisieTitre.getText()=="" || saisieTitre.getText()==null) {
				System.out.println("Veuillez donner un titre � l'�v�nement");
				return;
			}
			if(saisieDescriptif.getText()=="" || saisieDescriptif.getText()==null) {
				System.out.println("Veuillez ajouter une description � l'�v�nement");
				return;
			}
			if(image=="" || image==null) {
				System.out.println("Veuillez ajouter une image � l'�v�nement");
				return;
			}
		...
		if(parEvt.getSource()==inputImage) {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
	            ...
	            image = nomImage[0];
	            System.out.println("Fichier selectionn�: " + fileImage.getName());
	        } else {
	        	System.out.println("Ouverture de fichier annul�e par l'utilisateur");
	        }
		}
	}
}